...
def main():
    ...
    env, task = setup_env(headless=True, planning=True)  # Set headless=False to get an animation. (Slows down learning)

    # Setup of the agent
    print('Setting up the agent')
    save_model = False  # Do not set to true, model is trained already!
    model_dir = '/models/trained_model_50ms_visual_selector_test.pt'
    load_model = True  # If not loaded it starts to train from scratch
    trained_model_dir = '/models/trained_model_50ms_visual_selector_new_hope.pt'
    ...
    hparams = {
        ...
        "OPTIM_SCH": False, # Enables the use of a learning rate scheduler to automate the fine training
        "OPTIM_STEP": 300 # Optim steps for the lr scheduler
    }

    # Set up training
    tracker = training_tracker.Tracker(num_episodes=300,  # Either for training or for testing
                                       episode_length=1,  # Training length (1 for predict_only_first=True)
                                       enable_eval=False,  # Evaluation during training
                                       test_num=10,  # Amount of tests during eval cycles
                                       test_length=60,  # Size of the eval episodes
                                       test_intervals=15,  # Frequency of evaluation
    ...
    try:
        ...
        agent.plot(limit=True, y_range=0.004)
        tracker.plot()
