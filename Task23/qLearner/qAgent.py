...
class QAgent(object):
    ...
    def __init__(self, action_size, layer_size, hparams, load_model, model_dir=''):
        ...
        # Set up RL
        self.optimizer = optim.Adam(self.policy.parameters(), self.hparams['LEARNING_RATE'], weight_decay=5e-4)
        self.scheduler = None
        if self.hparams['OPTIM_SCH']:
            self.scheduler = schedules.StepLR(self.optimizer, step_size=self.hparams['OPTIM_STEP'], gamma=0.1)
        self.memory = replayMemory.ReplayMemory(self.hparams['BATCH_SIZE'])
        self.opt_counter = 0
    ...
    def optimize(self):
        ...
        if self.hparams['OPTIM_SCH']:
            self.scheduler.step()
            print('Current LR:', self.scheduler.get_last_lr()[0])
        self.policy.eval()
