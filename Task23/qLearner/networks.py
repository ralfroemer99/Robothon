...
class NAF(nn.Module):
    ...
    def __init__(self, action_size, layer_size):
        ...
        self.front_in, self.front_out = self.visual_model()
        self.front_hidden = self.visual_hidden()
        self.left_in, self.left_out = self.visual_model()
        self.left_hidden = self.visual_hidden()
        self.right_in, self.right_out = self.visual_model()
        self.right_hidden = self.visual_hidden()

        self.action_size = action_size
        visual_latent_size = 128 * 3 + 7 + 7
        self.hidden1 = self.hidden(visual_latent_size)
        self.hidden2 = self.hidden(visual_latent_size)
        self.hidden3 = self.hidden(visual_latent_size)
        self.hidden4 = self.hidden(visual_latent_size)
        self.clas = nn.Sequential(
            nn.Linear(visual_latent_size, action_size),
